...
#'
#'
V_Av.<-function(px,x,h,n,k=1,r,cantprem=1,premperyear=1,i=0.04,data,prop=1,assumption="none",variation="none",cap,t){
    ...
    if(k==1 && premperyear==1){
      if(t<=(h+n)){
        for(j in 1:t){
          ...
          prem<-px
          if(j>h){
            risk<-A.(x+j-1,0,1,1,i,data,prop,"none",1)*cap*(1+r*(j-h-1))
          }
          if(j>cantprem){
            prem<-0
          }
        ...
        if(variation=="inter"){
          for(s in 1:t){
            ...
            if(s>h*12 ){
              ...
              risk<-(cap)*(1+r*CumVariationInter)*qfrac(x+age,deferral,12,i,data,assumption,prop)*v^((Payment_Period-Elapsed_t_Payment)/12)
              deferral<-deferral+1
          ...
          for(s in 1:t){
            ...
            if(s>h*12 ){
              ...
              risk<-(cap)*(1+r*CumVariationIntra)*qfrac(x+age,deferral,12,i,data,assumption,prop)*v^((Payment_Period-Elapsed_t_Payment)/12)
              deferral<-deferral+1
              Elapsed_t_Payment<-Elapsed_t_Payment+1
            }
            if(contmeses(s,premperyear)==1 & Premiums_Paid<cantprem){
