#' @title  Reserve for Life Insurance

#' @description Calculates the reserve for the life insurance up to the moment 't'.
...
#' @keywords Reserve Life Insurance
    ...
    if(k==1 && premperyear==1){
      ...
      if(t<=(h+n)*12){
        ...
        for(s in 1:t){
          ...
          age<-trunc((s-1)/12)
          if(s>h*12 ){
            risk<-(cap)*qfrac(x+age,deferral,12,i,data,assumption,prop)*v^((Payment_Period-Elapsed_t_Payment)/12)
            deferral<-deferral+1
